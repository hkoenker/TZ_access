...
```{r read, message=FALSE, warnings=FALSE, echo=FALSE, fig.cap="Outbreaks in 2023"}
z <- st_read("/Users/hannahkoenker/Dropbox/R Directory/TZ_access/Unguja_Shehia_Corrected_Update", quiet=TRUE)
...
```{r outbreak-map, fig.cap="Mass Campaign Timing"}
z %>% 
ggplot() +
  ...
  geom_label_repel(data=filter(z, outbreak=="outbreak"), aes(label=Ward_Name, geometry=geometry), seed=2, fill="whitesmoke", label.padding=0, label.size = NA, color="black", stat="sf_coordinates", force=15, box.padding=.75) +
  labs(fill="",
       title="Shehias with increases in malaria 2023")
```
...
z %>% 
  ...
  labs(fill="shehias receiving mass campaign",
       title="Timing of ITN mass campaigns") +
