...
dfr <- read_dta("~/Dropbox/A DHS MIS Datasets/Analysis/TVCA/Access Estimates/output/tz_region_crop_npc_acc.dta") %>% 
  mutate(donor = factor(donor, labels = c("PMI","Global Fund"))) %>% 
  select(region, donor, year, accrk, access2, time)
...
dfr %>%
  ...
  geom_point(aes(x = year,
             y = accrk), fill = "orange", pch = 21, alpha = 0.3) +
  geom_point(aes(x = time,
             y = access2), fill = "green", pch = 21, alpha = 0.3) +
  #geom_smooth(method = "lm", alpha = .3) +
  geom_line(aes(x = accrk,
                y = accrk), alpha = .2) +
  scale_fill_discrete(guide = "none") +
